...
const StdQuiz = () => {
  ...
  const fetchQuizData = useCallback(async () => {
    try {
      ...
      setTotal(parsedData.questions.length)
    } catch (error) {
      console.log(error);
    }
  }, []);
  ...
  const handleSubmit = async (event) => {
    ...
    if (validateAnswers()) {
      ...
      // console.log("answers selected : ", formattedAnswers)      
  ...
  if(isQuizSubmitted) return <ShowScore score={score} total={total} />
